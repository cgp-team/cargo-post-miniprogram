...
Page({
  ...
  async monitorTick() {
    ...
    try {
      // 轮询走 silent 通道：弱网行车每 3s 弹一次"网络连接失败"toast 不可接受
      const pos = await api.getDriverPosition(this.driverId, { silent: true })
      if (!pos) return
  ...
  async reportRealLocation() {
    ...
    try {
      const loc = await location.getDeviceLocationGcj02()
      if (!loc || !loc.success) return // 定位失败/未授权：本轮放弃，静默等下个周期
      // 上报走 silent 通道：弱网失败不弹 toast
